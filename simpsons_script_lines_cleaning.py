"""
simpsons_script_lines_cleaning.py
──────────────────────────────────
Cleans and enriches the script-lines join table that links episodes,
...
3.  Type casting  (ids → Int64, booleans, timestamps)
4.  Fix speaking_line flag (coerce various truthy representations)
5.  Clean raw_text / spoken_words / normalized_text
      – strip leading/trailing whitespace
      – collapse internal whitespace runs
      – remove residual Wikipedia-style citation artefacts  [N]
6.  Recompute word_count from normalized_text and flag mismatches
7.  Fix raw_character_text / raw_location_text  (title-case ALL-CAPS)
8.  Validate foreign keys against the already-cleaned CSVs
9.  Derived columns
      – is_speaking  (alias for cleaned speaking_line, bool)
      – line_length  (character count of spoken_words)
      – has_dialogue (True when spoken_words is not empty/null)
10. Sort & reset index
11. Integrity assertions
12. Save to outputs/simpsons_script_lines_clean.csv
"""
...
INPUT_FILE = "simpsons_script_lines.csv"

df = pd.read_csv(INPUT_FILE, low_memory=False)

print("=" * 60)
print("SCRIPT LINES – CLEANING REPORT")
print("=" * 60)
print(f"\nRaw shape          : {df.shape}")
...
print(f"Fully-empty rows dropped : {n_dropped_empty}")
...
print(f"\nspeaking_line nulls after coercion : {n_null_speaking}")
print(f"speaking_line distribution         :\n{df['speaking_line'].value_counts(dropna=False).to_string()}")
...
# Flag rows where the original count differs from the recomputed one
mismatch_mask = (
    df["word_count"].notna()
...
# Replace with recomputed value (more reliable after text cleaning)
df["word_count"] = df["word_count_recomputed"]
...
out_path = "outputs/simpsons_script_lines_clean.csv"
